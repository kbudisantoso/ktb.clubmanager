...
describe('RejectionNotice', () => {
  ...
  describe('rejection reasons', () => {
    it.each([
      ...
      render(<RejectionNotice request={createMockRequest({ rejectionReason: reason })} />);
    ...
    it('shows rejection note when provided', () => {
      ...
      expect(screen.getByText(/"Bitte melde dich telefonisch"/)).toBeInTheDocument();
    });

    it('does not show note section when no note provided', () => {
      render(<RejectionNotice request={createMockRequest({ rejectionNote: undefined })} />);
  ...
  describe('retry action', () => {
    it('shows retry button when onRetry is provided', () => {
      const onRetry = vi.fn();
      render(<RejectionNotice request={createMockRequest()} onRetry={onRetry} />);
  ...
  describe('re-request info', () => {
    it('shows info that user can request again', () => {
      ...
      expect(screen.getByText(/Du kannst jederzeit eine neue Anfrage stellen/)).toBeInTheDocument();
    });
  });
});
